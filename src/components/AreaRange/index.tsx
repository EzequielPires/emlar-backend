...
export function AreaRange({title}) {
    const [min, setMin] = useState('260000');
    const [max, setMax] = useState('750000');
    const [minInput, setMinInput] = useState('260.000,00 m²');
    const [maxInput, setMaxInput] = useState('750.000,00 m²');
    ...
    const handleOnChange = (e) => {
        if ((renge_last.current.value - renge_first.current.value) < 1000) {
            ...
            setMinInput( VMasker.toMoney(renge_first.current.value + '00') + ' m²');
            setMaxInput( VMasker.toMoney(renge_last.current.value + '00') + ' m²');
